...
public class AcknowledgementService {
    ...
    public AcknowledgementService(ContentRepository contentRepository, AuthorizationPolicy authorizationPolicy, AuditService auditService, CmsMapper mapper) {
    ...
    public List<UserAcknowledgementItem> myAcknowledgements(String status, CmsSecurityContext context) {
        return contentRepository.list().stream()
            .filter(item -> item.isPortalVisibleNow() && item.isRequiresAcknowledgement())
            .map(item -> {
    ...
    public AcknowledgementReport report(String contentId, String department, String status, CmsSecurityContext context) {
        if (!authorizationPolicy.canViewReports(context)) {
            throw ApiException.forbidden("Only editors or administrators can view acknowledgement reports");
        }
        var item = contentRepository.get(contentId);
        ...
        var rows = users.stream()
            ...
            .toList();
        int acknowledgedCount = (int) rows.stream().filter(row -> row.status().equals("COMPLETED")).count();
        int targeted = rows.size();
